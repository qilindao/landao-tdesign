...
const { setOption: setMoneyChartOption, getInstance: getMoneyChartInstance } =
  useECharts(moneyContainer);

const refundContainer = ref(null);
const { setOption: setRefundChartOption, getInstance: getRefundChartInstance } =
  useECharts(refundContainer);

const renderCharts = () => {
  setMoneyChartOption(constructInitDashboardDataset("line"));

  setRefundChartOption(constructInitDashboardDataset("bar"));
};
...
const updateContainer = () => {
  ...
  getMoneyChartInstance().resize({
    width: resizeTime.value * 120,
    // height: resizeTime.value * 100,
  });
  getRefundChartInstance().resize({
    width: resizeTime.value * 120,
